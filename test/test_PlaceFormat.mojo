...
def test_PlaceFormat_1():
    ...
    assert_equal(place.map[0, 0]["A"][0], Block("A", 0, Blocktype.NONE), "Block A in map nicht valide")
    assert_equal(place.map[0, 1]["B"][0], Block("B", 0, Blocktype.NONE), "Block B in map nicht valide")
    assert_equal(place.map[1, 0]["C"][0], Block("C", 0, Blocktype.NONE), "Block C in map nicht valide")
    assert_equal(place.map[1, 1]["D"][0], Block("D", 0, Blocktype.NONE), "Block D in map nicht valide")
